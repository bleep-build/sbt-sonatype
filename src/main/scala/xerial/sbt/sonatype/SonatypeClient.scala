...
class SonatypeClient(
  ...
  def createStage(profile: StagingProfile, description: String): StagingRepositoryProfile = {
    ...
    logger.info(s"Created successfully: ${repo.repositoryId}")
    repo
  }
      ...
      .beforeRetry { ctx =>
        ctx.lastError match {
          case SonatypeException(STAGE_IN_PROGRESS, msg) =>
            logger.info(f"${msg} Waiting for ${ctx.nextWaitMillis / 1000.0}%.2f sec.")
          case _ =>
            logger.warn(
  ...
  def closeStage(currentProfile: StagingProfile, repo: StagingRepositoryProfile): StagingRepositoryProfile = {
    ...
    waitForStageCompletion(
      "close",
      repo,
      terminationCond = _.isCloseSucceeded(repo.repositoryId)
    ).toClosed
  }
...
object SonatypeClient {
  ...
  case class ActivityEvent(timestamp: String, name: String, severity: Int, properties: Seq[Prop]) {
    ...
    def showProgress(logger: Logger, useErrorLog: Boolean = false): Unit = {
      val props = {
        val front =
  ...
  class ActivityMonitor {
    ...
    def report(logger: Logger, stagingActivities: Seq[StagingActivity]) =
      for (sa <- stagingActivities) {
        ...
        for (ae <- sa.events if !reportedEvents.contains(ae)) {
          ae.showProgress(logger, useErrorLog = false)
          reportedEvents += ae
        }
      }
  }
  ...
  case class StagingActivity(name: String, started: String, stopped: String, events: Seq[ActivityEvent]) {
    ...
    def reportFailure(logger: Logger): Unit = {
      logger.error(activityLog)
      for (e <- failureReport)
        e.showProgress(logger, useErrorLog = true)
    }
